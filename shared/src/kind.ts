...
/**
 * The first segments that are not a chapter id: the two entry prefixes and
 * the three LIST segments. The lists are reserved so no chapter can claim one
 ...
 * describe is `unknown`.
 *
 * Only the five ENTRY kinds have an address. `inbox`, `glossary` and
 * `sessions` are reserved segments and nothing else (ADR #26) — they are
 * lists with their own endpoints — so an address reaching for one of them
