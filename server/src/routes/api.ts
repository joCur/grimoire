...
api.put("/campaigns/:campaign/chapters/:chapter/scene-order", async (c) => {
...
api.get("/campaigns/:campaign/chapters/:chapter/threads", async (c) =>
...
// „Handlungsstrang übernehmen" in the review and behind adding a thread in
...
api.post("/campaigns/:campaign/chapters/:chapter/threads", async (c) => {
  const body = await jsonBody(c, ["text"]);
  const text = normalizeLineText(body.text);
  if (text === undefined) throw new ApiError(400, "text must be a non-empty string");
...
// the token stands.
api.patch("/campaigns/:campaign/chapters/:chapter/threads/:id", async (c) => {
...
api.delete("/campaigns/:campaign/chapters/:chapter/threads/:id", async (c) => {
