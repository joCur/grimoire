...
// neighbours sit too.
//
// `rev` is the CHAPTER's guard token (the one the chapter entry hands out),
// and the answer carries the fresh one. That is the right guard because the
// order belongs to the chapter; a stale one is
// 409 { code: "rev_conflict", rev } and writes nothing. No `entry` rides
