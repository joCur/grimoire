...
// time. The intervals travel in the response (store/render.ts), so the
// assertions read the RESPONSE — and the log is asserted to stay EMPTY, which
// is what says the marker rows are gone. Each case
...
describe("POST /api/campaigns/:campaign/session/pause + /continue", () => {
  test("pause opens an interval, continue closes it — and neither writes a log row", async () => {
    ...
    expect(logLines(paused)).toEqual([]);
    ...
    expect(logLines(running)).toEqual([]);
  ...
  test("both calls are idempotent — no second interval", async () => {
    ...
    expect(logLines(stillRunning)).toEqual([]);
  });
