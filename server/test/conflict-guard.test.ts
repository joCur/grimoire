...
import type { Scene, ScenePatch } from "@grimoire/shared";
...
async function patchReq(request: ScenePatch | Record<string, unknown>): Promise<Response> {
  return app.request(SCENE, {
    method: "PATCH",
...
describe("two writes with the same guard token, same clock second", () => {
  test("a field: first wins, second is 409, the loser did not land", async () => {
    const read = await getScene(SCENE);

    // Both "tabs" hold the SAME token — the one read above.
    const first = await patchReq({ rev: read.rev, status: "played" });
    expect(first.status).toBe(200);
    const won = (await first.json()) as Scene;
    expect(won.status).toBe("played");
    expect(won.rev).toBe(read.rev + 1);

    const second = await patchReq({ rev: read.rev, status: "draft" });
    ...
    // the current SCENE, so it can show what is in the way right away.
    expect(conflict.rev).toBe(won.rev);
    expect(conflict.scene).toEqual(won);
    ...
    const retry = await patchReq({ rev: conflict.rev, status: "draft" });
    expect(retry.status).toBe(200);
    const retried = (await retry.json()) as Scene;
  ...
  test("body: first wins, second is 409, the loser's body did not land", async () => {
    ...
    const conflict = (await second.json()) as Conflict;
    expect(conflict.rev).toBe(won.rev);
    // The conflict body is the winner's scene, text included: what the app
    // shows next to the DM's own version.
    expect(conflict.scene.body).toBe("\n## Flow\n\nVersion A.\n");
    ...
    expect(retry.status).toBe(200);
    expect(((await retry.json()) as Scene).body).toBe("\n## Flow\n\nVersion B.\n");
  });
  ...
  test("fired together: exactly one lands, whichever the runtime schedules first", async () => {
    ...
    const responses = await Promise.all([
      patchReq({ rev: read.rev, status: "played" }),
      patchReq({ rev: read.rev, status: "ready" }),
    ]);
    ...
    const won = (await okRes.json()) as Scene;
  ...
  test("fields and text share one token — a saved field invalidates a held text", async () => {
    ...
    const read = await getScene(SCENE);
    const patched = await patchReq({ rev: read.rev, status: "played" });
    ...
    expect((await getScene(SCENE)).status).toBe("played");
  });

  test("a field and the text in one request move the rev once and cannot half-land", async () => {
    const read = await getScene(SCENE);
  ...
  test("force writes the held fields on top of the current row", async () => {
    ...
    // request carries.
    const read = await getScene(SCENE);
    const other = await patchReq({ rev: read.rev, status: "played" });
