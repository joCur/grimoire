...
//     `marked: false`. The pre-database version appended the hash regardless and
...
describe("POST /api/campaigns/:campaign/review/seen", () => {
  ...
  test("adds the short hash once; Log unchanged; key rendered last", async () => {
    const before = await getEntry(SESSION);
    const entry = await postOk("/api/campaigns/beispiel/review/seen", { path: SESSION, line: LINE });
    ...
    // never rewrites it.
    expect(entry.body).toBe(before.body);
  });
  ...
  test("hash matches an independently computed sha-256 prefix", async () => {
    ...
    const entry = await postOk("/api/campaigns/beispiel/review/seen", { path: SESSION, line: LINE });
    expect(entry.properties.reviewed).toEqual([expected]);
  });
  ...
  test("several lines render in LOG order, not in the order they were seen", async () => {
    // Deliberately marked back to front. The pre-database version appended hashes as
  ...
  test("a line that is not in the log changes nothing and says so", async () => {
    ...
    const { marked: _marked, ...entry } = after;
