// Editing ONE draft of a generator review: its properties in the form of the
// properties dialog, its body on the surfaces of the entry editor.
//
// A draft is not an entry yet, so this component brings none of the write
...
//
//   properties  propertiesFieldsFor + PropertiesFieldControl — the same field
//               list, labels, controls and per-field notes as the dialog. The
...
// is never sent (shared, DraftEdit).
//
// The form's state is seeded ONCE, so this is mounted per draft address (`key`)
// the way the dialog is mounted per entry: re-reading the properties out of the
// props would fight the keystroke that produced them.
//
...
/**
 * A DOM id that survives any address (same rule as the entry editor): the
...
export function DraftEditor({
  ...
  /** The address the draft would be written to — the read-only id line. */
  path: string;
  /** Which field list the draft gets; a kind without a form shows none. */
  kind: EntityKind;
  /** The draft's properties as the review holds them when the editor opens. */
  properties: Record<string, unknown>;
  /** The draft's body as the review holds it when the editor opens. */
  body: string;
