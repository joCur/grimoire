// One editing session over the campaign (decisions/resources), with the rules every
// editing session in the app follows:
//
//   the write       ONE PATCH of the campaign per interaction — any subset of
//                  its fields, `body` among them.
//   the version     taken when the edit STARTS and moved only by a successful
//                  write or by the DM adopting the stored campaign. The 5s
...
//                  refused fields; `conflict` carries the stored campaign, and
...
import { campaignConflict, patchCampaign, type CampaignConflict } from "./campaign-api";
import { campaignKey } from "./campaign-query";
...
export interface CampaignEdit {
  ...
  save: (change: CampaignChange) => void;
  isSaving: boolean;
  /** Set while the last write stands refused; `campaign` is what is stored now. */
  conflict?: { campaign: Campaign | undefined } | undefined;
  /** Drop the draft and continue from the stored campaign (`onReload` reseeds). */
  reload: () => void;
...
export interface CampaignEditOptions {
  /** Runs after a SUCCESSFUL write — where a dialog closes or edit mode ends. */
  onSaved: () => void;
  /** Runs when the DM adopted the stored campaign, so the surface can reseed. */
  onReload?: (campaign: Campaign) => void;
...
export function hasCampaignChange(change: CampaignChange): boolean {
  return Object.values(change).some((value) => value !== undefined);
}

/**
 * `campaign` is the campaign as it was read when the edit started. Mount the
 * surface per campaign (`key`) so a switch starts a new session.
 */
export function useCampaignEdit(
  ...
  const [rev, setRev] = useState(campaign.rev);
  const [refused, setRefused] = useState<{ change: CampaignChange; conflict: CampaignConflict }>();
  ...
  const mutation = useMutation({
    mutationFn: ({ change, force }: { change: CampaignChange; force: boolean }) => {
      const request: CampaignPatch = { rev, ...change, ...(force ? { force: true } : {}) };
      return patchCampaign(campaign.id, request);
    },
    onMutate: () => {
    ...
    onError: (error, variables) => {
      const conflict = campaignConflict(error);
  ...
  const start = (change: CampaignChange, force: boolean) => {
    if (inFlight.current || mutation.isPending) return;
    inFlight.current = true;
    mutation.mutate({ change, force });
  };

  const stored = refused?.conflict.campaign;

  return {
    rev,
    save: (change) => {
      if (!hasCampaignChange(change)) return;
      start(change, false);
    },
    isSaving: mutation.isPending,
    ...(refused === undefined ? {} : { conflict: { campaign: stored } }),
    reload: () => {
      if (stored === undefined) {
        // No campaign in the 409 body: let the query fetch the truth.
