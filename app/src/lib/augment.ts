...
/**
 * An npc proposal, field by field (ADR #31): what the model changes against
 * the npc as the run read it — the same rule as `locationFieldProposals`.
 * Only a field that CHANGES is listed, a field the proposal leaves empty is
 * no deletion, the `id` never changes, and `body` is reviewed block by block.
 * `status` always holds a value, so a proposed status is a `changed` field
 * whose default is to keep the stored one.
 */
