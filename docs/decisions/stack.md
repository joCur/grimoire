...
- **Deployment:** one container with a data volume
  ([docs/DEPLOYMENT.md](../DEPLOYMENT.md)). No desktop shell.
...
- A web app covers every device the DM uses; a desktop shell would add
  packaging without benefit.

## Consequences

- The Bun-specific risk is limited to the registered couplings.
- The runtime does not limit growth; the limits are auth
  (`decisions/scope`), data volume (`decisions/sqlite`) and multi-user (the
  data model).
