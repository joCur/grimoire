...
- `app/` — das Frontend. Jede Entität mit eigener Ressource hat ihren
  ...
  (decisions/resources); **Slices importieren einander nicht.** Pause und
  Log-Zeile gehören zum Slice `session/`: die App liest sie nur eingebettet in
  ihrer Session, und jeder ihrer Schreibzugriffe landet im Cache der Session;
  ihre Ressourcen haben dort je ein eigenes Modul (`pause-api.ts`,
...
- Kampagne, Kapitel, Szene, NPC, Ort, Faden, Idee, Glossar-Begriff,
  ...
  eigene Ressource (decisions/resources):
